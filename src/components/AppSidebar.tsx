...
const ROLE_COLOR: Record<string, { ring: string; badge: string; dot: string }> = {
  ...
  secretary:      { ring: 'ring-rose-500',    badge: 'bg-rose-500/15 text-rose-700 dark:text-rose-300 ring-rose-500/30',             dot: 'bg-rose-500' },
  auxiliary:      { ring: 'ring-amber-500',   badge: 'bg-amber-500/15 text-amber-700 dark:text-amber-300 ring-amber-500/30',          dot: 'bg-amber-500' },
...
export function AppSidebar() {
  ...
  // — but unlocked when the clinic admin switches to Modo Consulta
  const PROFESSIONAL_ONLY_URLS = ['/minha-agenda', '/disponibilidade'];
  const blockProfessional = isClinicSignup && !isConsultMode;
