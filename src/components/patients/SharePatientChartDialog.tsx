...
export function SharePatientChartDialog({ patientId, patientName, patientPhone, open, onOpenChange }: Props) {
  ...
  const generate = async () => {
    setLoading(true);
    try {
      const { data, error } = await supabase.functions.invoke('share-patient-chart', {
  ...
  const whatsapp = () => {
    if (!code) return;
    const msg = `Olá! Estou compartilhando o prontuário de ${patientName}.\n\nAcesse: ${shareUrl}\nCódigo: ${code}\n\nEste código expira em 5 minutos.`;
        ...
        {!code ? (
          <div className="space-y-4 py-2">
            <p className="text-sm text-muted-foreground">
              Gere um código de acesso temporário para que outro profissional possa baixar o prontuário completo de <b className="text-foreground">{patientName}</b>.
            </p>
            <div className="rounded-lg border border-amber-200 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-900 p-3 text-xs text-amber-900 dark:text-amber-200 flex gap-2">
              ...
              <span>O código expira em <b>5 minutos</b> e dá acesso ao prontuário completo. Compartilhe apenas com o profissional destinatário.</span>
            </div>
