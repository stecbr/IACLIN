  ...
  const { data: noShowRate = 0 } = useQuery({
    ...
    queryFn: async () => {
      ...
      let q2 = supabase
        ...
        .eq("status", "no_show");
      if (currentClinicId) {
        q1 = q1.eq("clinic_id", currentClinicId);
        q2 = q2.eq("clinic_id", currentClinicId);
      }
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {kpiCards.map((kpi, i) => (
          ...
          >
            ...
            <div
              className={`absolute inset-0 bg-gradient-to-br ${kpi.gradientFrom} ${kpi.gradientTo} opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
            />
            <CardHeader className="relative flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">{kpi.title}</CardTitle>
              <div
                className={`h-9 w-9 rounded-xl ${kpi.iconBg} flex items-center justify-center transition-transform duration-300 group-hover:scale-110`}
              >
              ...
              <div className="flex items-end justify-between">
                <div>
                  <AnimatedNumber
                    ...
                    formatter={
                      kpi.isCurrency ? (v) => fmt(v) : kpi.isPercentage ? (v) => `${Math.round(v)}%` : undefined
                    }
                  />
                  <p className="mt-1 text-xs text-muted-foreground">{kpi.description}</p>
                </div>
                <MiniSparkline
                  data={last7DayCounts.length > 0 ? last7DayCounts : [0, 0]}
            ...
            {upcoming.length === 0 ? (
              ...
              <div className="space-y-1">
                {upcoming.map((apt: any) => (
                  <div
                    key={apt.id}
                    className="flex items-center gap-3 py-2.5 px-2 rounded-lg border-b border-border/30 last:border-0 hover:bg-muted/40 transition-colors"
                  >
            ...
            <CardContent>
              <div className="space-y-1">
                {pendingPayments.map((tx: any) => (
                  <div
                    key={tx.id}
                    className="flex items-center justify-between py-2.5 px-2 rounded-lg border-b border-border/30 last:border-0 hover:bg-muted/40 transition-colors"
                  >
                    <div>
                      <p className="text-sm font-medium text-foreground">{tx.patients?.full_name ?? tx.description}</p>
                      <p className="text-xs text-muted-foreground">
                        Venc: {format(parseISO(tx.due_date), "dd/MM/yyyy")}
                      </p>
                    </div>
        ...
        <div className="flex flex-wrap gap-3">
          {[
            ...
            <Button
              key={action.label}
              variant="outline"
              className="gap-2 border-border/50 hover:bg-accent shadow-card hover:shadow-card-hover transition-all"
              asChild
            >
              <Link to={action.href}>
