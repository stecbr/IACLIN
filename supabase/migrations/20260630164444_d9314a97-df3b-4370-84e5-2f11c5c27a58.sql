...
-- 1. Add platform account flag
ALTER TABLE public.insurance_operators
  ADD COLUMN IF NOT EXISTS is_platform_account boolean NOT NULL DEFAULT false;

-- Backfill: anything with an owner is a real account
UPDATE public.insurance_operators
SET is_platform_account = true
WHERE owner_id IS NOT NULL AND is_platform_account = false;

CREATE INDEX IF NOT EXISTS insurance_operators_is_platform_account_idx
  ON public.insurance_operators (is_platform_account);

-- 2. admin_get_operators now returns ONLY real platform accounts (signup requests + approved + rejected)
...
-- 3. handle_new_user: claim existing catalog operator if possible, otherwise insert new
